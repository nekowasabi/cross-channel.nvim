...
import { aider } from "./crossChannelCommand.ts";
import { getCurrentFilePath, getPromptFromVimVariable, postToBluesky } from "./utils.ts";
...
export async function sendPromptFromFloatingWindow(
  denops: Denops,
  prompt: string,
): Promise<void> {
  const aiderBuf = await getAiderBuffer(denops);
